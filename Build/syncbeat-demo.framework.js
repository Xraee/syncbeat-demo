...
// end include: preamble.js
  ...
  function _DownloadFile(filenamePtr, contentPtr) {
          var filename = UTF8ToString(filenamePtr);
          var content  = UTF8ToString(contentPtr);
  
          // Wrap the content in a Blob so the browser treats it as a file.
          var blob = new Blob([content], { type: 'text/csv;charset=utf-8;' });
          var url  = URL.createObjectURL(blob);
  
          // Create a temporary <a> element, click it to trigger the download,
          // then immediately remove it and revoke the object URL to free memory.
  ...
  function _MicInput_Start(goNamePtr) {
          var goName = UTF8ToString(goNamePtr);
  
          window._syncBeatMic = window._syncBeatMic || {};
          window._syncBeatMic.goName = goName;
  
          if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
              console.warn('[SyncBeat] getUserMedia not supported in this browser.');
              SendMessage(goName, 'OnMicError', 'getUserMedia not supported');
              return;
          }
  
          navigator.mediaDevices.getUserMedia({ audio: true, video: false })
              .then(function (stream) {
                  var m = window._syncBeatMic;
                  m.stream = stream;
  
                  var ctx = new (window.AudioContext || window.webkitAudioContext)();
                  m.ctx = ctx;
  
                  // AudioContext may start suspended — resume it (requires user gesture,
                  // which we have since this is triggered by a key press).
                  ctx.resume();
  
                  var source = ctx.createMediaStreamSource(stream);
  
                  // ScriptProcessorNode: 1024-sample buffer, 1 input channel, 1 output channel.
                  // Fires ~43 times/second at 44100 Hz — enough resolution for clap detection.
                  ...
                  proc.onaudioprocess = function (e) {
                      ...
                      SendMessage(window._syncBeatMic.goName, 'OnRmsUpdate', rms.toString());
                  };
  
                  // Connect: source → processor → destination (silent output required
                  // for ScriptProcessorNode to keep firing in some browsers).
                  source.connect(proc);
                  proc.connect(ctx.destination);
  
                  SendMessage(goName, 'OnMicReady', '');
              })
              .catch(function (err) {
                  console.warn('[SyncBeat] Microphone error: ' + err);
                  SendMessage(window._syncBeatMic.goName, 'OnMicError', err.message || err.toString());
              });
      }

  function _MicInput_Stop() {
          var m = window._syncBeatMic;
          if (!m) return;
          if (m.proc)   { m.proc.disconnect();   m.proc   = null; }
          if (m.stream) { m.stream.getTracks().forEach(function (t) { t.stop(); }); m.stream = null; }
...
var wasmImports = {
  ...
  "MicInput_Start": _MicInput_Start,
  "MicInput_Stop": _MicInput_Stop,
